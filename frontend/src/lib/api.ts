...
export type BlogComment = Schemas["BlogComment"];
...
export const api = {
  ...
  blog: {
    ...
    posts: (
      ...
      signal?: AbortSignal,
    ) => apiFetch<BlogPostSummary[]>("/blog-posts", { query: params, signal }),
    post: (id: string, signal?: AbortSignal) => apiFetch<BlogPost>(`/blog-posts/${id}`, { signal }),
    /** The public site's URL shape (/blog/$slug) — same reasoning as listings.bySlug. */
    postBySlug: (slug: string, signal?: AbortSignal) =>
      apiFetch<BlogPost>(`/blog-posts/by-slug/${slug}`, { signal }),
